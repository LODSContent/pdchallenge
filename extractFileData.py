...
filePath = "C:\\challenge-files-github\\python-data\\data\\{}.{}"
...
def saveCustomer():
    ...
    output = csr.fetchone()
    csr.close()
    with open(filePath.format("customer","json"),"w") as f:
...
def saveProducts():
    ...
    with open(filePath.format("products","json"),"w") as f:
        for product in csr:
...
def saveOrders():
    ...
    for month in months:
        ...
        with open(filePath.format("{}-{}".format(month.orderYear, month.orderMonth),"csv"),"w") as f:
            for order in csr:
